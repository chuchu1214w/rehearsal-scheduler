...
**当前状态:M0(求解包)、M1(账号、演出、人员、曲目、特殊要求)、M2 的大部分(成员涂格填报、管理员进度与代填)与 M3(一键求解、无解诊断、排练表草稿版本与列表视图)已完成,界面按 `design/手机UI草图` 实现;排练表的周视图、拖拽微调、发布与日历订阅(M4)尚未接入。** 交互与视觉依据见 [交互设计.md](交互设计.md),里程碑与设计见 [开发文档.md](开发文档.md)。
