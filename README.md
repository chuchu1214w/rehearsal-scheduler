...
**当前状态:M0(求解包)、M1(账号、演出、人员、曲目、特殊要求)、M2 的大部分(成员涂格填报、管理员进度与代填)、M3(一键求解、无解诊断)、M4(课程表式周日历、当天日程、发布、成员端我的 / 全体切换、日历订阅)、M5(拖动移动、锁定后重排、版本对比、空闲变更冲突提示)与 M6(站内通知中心、一键催办、定时提醒、优化优先级面板、规则冲突检查、端到端自动化测试)已完成,界面按 `design/手机UI草图` 实现;M7 的部署材料(Dockerfile、fly.toml、docker-compose + Caddy、自动备份)已就绪,上线步骤见 [部署指南.md](部署指南.md)。** 交互与视觉依据见 [交互设计.md](交互设计.md),里程碑与设计见 [开发文档.md](开发文档.md)。
