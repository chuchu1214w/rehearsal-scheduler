...
> **版本** v1.3 · **日期** 2026-09-21 · **状态** 需求已确认;**M0、M1 已完成**,M2 起未开始
...
### 7.3 视觉规范(UI 紧扣舞团 logo)

参考图:[design/logo-reference.png](design/logo-reference.png);设计令牌:[design/tokens.css](design/tokens.css)(M4 起前端直接引入)。logo 的风格特征与界面映射如下:

| logo 特征 | 界面映射 |
|---|---|
| 浅粉底 `#fcd5f4`,泡泡粉字面 `#e59cc8` / `#f7a0d1`,藏青描边 `#5369af` | 页面底色用浅粉;主按钮、选中的时间格用泡泡粉;边框、链接、评估场用藏青 |
| 圆润的气泡字、白色高光 | 标题用圆体展示字(如 Baloo 2 / 站酷快乐体),卡片与按钮用大圆角(20px)和左上白色高光 |
| 错位的实心阴影(不模糊) | 卡片、按钮、日历事件块使用 `4px 4px 0 藏青` 的错位阴影;悬停时位移 2px |
| 星星、网点、格纹装饰 | 空状态、加载页、发布成功页用星星点缀;“尽量避开”时段用斜纹或网点区分,不只靠颜色(可访问性) |
| 只有粉、白、藏青三色 | 状态色(成功 / 警告 / 错误)限于图标与细节,不与品牌色抢主次 |

具体色值与深色模式见 `tokens.css`。**logo 文件本身需要你提供矢量或高清版本**(当前参考图是截图裁切,只用于确定配色与风格,不能直接放进界面)。
