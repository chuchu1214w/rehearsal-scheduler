...
> **版本** v1.7 · **日期** 2026-09-21 · **状态** 需求已确认;**M0、M1、M3 已完成**,M2 大部分完成,M4 起未开始
...
| `meta` | 键值表;`schema_version` 记录当前结构版本(现为 3)。启动时若库版本更旧,按 `app/db.py` 的 `MIGRATIONS` 注册表逐级升级;更新则拒绝启动 |
...
| | `POST /api/schedules/{vid}/publish` 及微调、锁定、重排、对比 | **(M4)** | 管理员 |
| | `GET /api/events/{id}/schedule/published`、`GET …/schedule/mine` | 已发布排练表 / 我的排练 **(M4)** | 全员 |
| 日历与通知 | `GET /cal/{token}.ics`、`POST /api/me/calendar-token/rotate`、`GET /api/notifications` … | 订阅、令牌重置、通知 **(M4)** | 令牌 / 登录用户 |
...
- 同一 Wi-Fi 下的成员可通过 `http://<你的电脑局域网地址>:8000` 访问;**电脑需保持开机、不休眠**。邀请链接默认指向 `localhost`,发给成员前需把 `PUBLIC_BASE_URL` 设为局域网地址。
...
| `PUBLIC_BASE_URL` | 生成邀请链接与 ICS 链接所用的对外地址,默认 `http://localhost:8000` |
