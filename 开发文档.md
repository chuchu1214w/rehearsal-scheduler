...
> **版本** v1.6 · **日期** 2026-09-21 · **状态** 需求已确认;**M0、M1 已完成**,M2 起未开始
...
| 数据访问 | SQLAlchemy 2;Alembic 迁移从 M2 起引入(M1 用 create_all 建表,尚无需迁移的真实数据) | 通过 `DATABASE_URL` 在 SQLite 与 PostgreSQL 间切换;相对路径的 SQLite 文件相对于仓库根目录 |
...
| | `POST /api/events/{id}/solve`、`GET /api/solve-jobs/{id}`、`…/stream`、`…/cancel`、`…/diagnosis` | 发起求解、进度、取消、诊断 **(M3)** | 管理员 |
| 排练表 | `GET /api/events/{id}/schedules` … `POST /api/schedules/{vid}/publish` | 版本、微调、锁定、重排、对比、发布 **(M4)** | 管理员 |
