...
> **版本** v1.9 · **日期** 2026-09-21 · **状态** 需求已确认;**M0、M1、M3、M4、M5 已完成**,M2 大部分完成,M6 起未开始
...
| `meta` | 键值表;`schema_version` 记录当前结构版本(现为 4:3→4 为 `users.calendar_token`)。启动时若库版本更旧,按 `app/db.py` 的 `MIGRATIONS` 注册表逐级升级;更新则拒绝启动 |
...
| 通知 | `GET /api/notifications` … | **(M6)** | 登录用户 |
