...
> **版本** v1.4 · **日期** 2026-09-21 · **状态** 需求已确认;**M0、M1 已完成**,M2 起未开始
...
- 移动优先:所有页面按 375–440px 宽设计;桌面浏览器上居中显示同样的窄列(最大 440px),不做桌面专属布局。
...
| | `GET /api/invites/{token}`、`POST /api/invites/{token}/accept` | 查看邀请 / 接受邀请并设置账号 | 公开 |
| 名册 | `GET/POST /api/members`、`PATCH/DELETE /api/members/{id}` | 名册管理 | 管理员 |
| | `POST /api/members/{id}/invite`、`POST /api/members/{id}/reset-password`、`PATCH /api/members/{id}/account` | 邀请 / 重置密码 / 停用或启用账号 | 管理员 |
